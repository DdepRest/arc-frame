...
а рисунок букв остаётся Inter'овский). Все литералы 13 → 12; единственный
оставшийся 13.5 — `Services/FlowDocumentBuilder` (печатный слой, пункты для
QuestPDF, не интерфейс). Стражи: `TypographyTests.TypeScale_ExcludesDegenerateThirteenPixelStep`
(нет `Type.BodyMd`, нет `>13<` в словарях, нет `FontSize="13"` в разметке) + шкала
