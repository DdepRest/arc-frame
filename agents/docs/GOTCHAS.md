...
прогонов, кадры `.tools/shots`, «+22 теста» — всё это читал заказчик).
...
`releases.json` с версии 3.50.0 — первой, которую увидит пользователь 3.49.0): маркеры — обратные кавычки, `§`/GOTCHAS, имена
...
`pre-commit`, который зовёт его в режиме `-Staged` и заново кладёт изменённые файлы
в индекс. Дата ставится ДНЁМ КОММИТА: именно с ней сравнивает #7, поэтому правка,
...
диагнозом, а хук на staged-документе вставил дату и заново положил файл в индекс).
