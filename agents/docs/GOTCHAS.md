...
- Импост добавляется к `Total` в `Recalculate()`, поэтому затрагивает `TotalWithDeduction`, КП и итоги (текст «На завод» удалён в v3.54).
...
- В `DisplayName` добавляется суффикс `(Импост)` — печатные суффиксы обязательны в КП и PDF (`PrintPipelineSuffixTests`).
...
пользовательскими — их не трогали. Четвёртый проход накрыл 3.44.0–3.47.4 (11
записей): два рекордсмена — 3.44.0 (66 пунктов / 8987 знаков) и 3.44.2 (69 /
13 162) — дублировали друг друга и вели диалог о тестах и ширинах колонок печатного
КП; 3.45.0 был рефакторинг-журналом с именем файла в заголовке. Теперь 3.44.0 —
13/1916, 3.44.2 — 7/1191, 3.45.0 — 3/373, остальные одиннадцати записей переведены
в тот же голос. Пятый, последний проход — хвост 3.40.0–3.43.2 (11 записей): 3.41.0
держал 130 пунктов на 36 265 знаков, а его заголовок был перечитан в чужой кодировке
— «РЈР»СѓС‡С€РµРЅРёСЏ» вместо «Улучшения» (двойной mojibake, восстановлен обратным
decode, cp1251→utf-8); 3.43.0/3.43.2 — по 32/33 пункта с именами тестов и счётчиками
(709/711, 789/789); в 3.42.1 — пути вида %AppData% и .bat-скрипты. Теперь 3.41.0 —
9/1275, 3.43.0 — 6/711, 3.43.2 — 6/748; почти чистые 3.40.x заодно избавлены от
«opt-in» и потерянных «до/после». Порог опущен до 3.40.0 — самой старой записи
журнала: страж теперь покрывает update-log.json ЦЕЛИКОМ, опускать дальше некуда.
Попутно выяснилось, что при опускании порога грязь всплывает в `releases.json`, а не
в переписанной записи (её читаешь и чистишь первой, а releases.json лежит рядом) —
там жили те же девжурналы 3.40.0–3.40.3, и их пришлось чистить тем же заходом.
...
`releases.json` с версии 3.40.0 — самой старой записью журнала, порог покрывает его
целиком): маркеры — обратные кавычки, `§`/GOTCHAS, имена
...
### 42. build.bat из bash/Git Bash: кириллический путь в cmd-батнике молча убивает сборку (СРЕДНИЙ)

**Симптом.** `build.bat`, запущенный из bash-терминатора (`cmd //c "build.bat"`),
отрабатывает `dotnet publish`, копирует tessdata/x64/x86 — и умирает на копировании
`docs\images\ВПРОИЗВОДСТВО.png` с exit code 1. В выводе ни одного сообщения об ошибке,
ZIP не создаётся, повторный запуск падает ровно там же.

**Причина.** cmd парсит батник в OEM-кодировке (cp866), а файл сохранён в UTF-8:
кириллический литерал в пути внутри батника не совпадает с реальным именем файла.
`copy /y` не находит источник, `if %errorlevel% neq 0 exit /b 1` молча завершает bat;
само сообщение cmd («Системе не удается найти указанный путь») теряется при
перекодировании вывода в bash. В PowerShell и CI проблема не воспроизводится.

**Решение.** `chcp 65001 >nul` в шапке самого `build.bat` (сразу после `@echo off`):
кодовая страница меняется до первого кириллического литерала, и тот же cmd дальше
парсит батник в UTF-8. Комментарий над `chcp` — только ASCII, чтобы cmd не встретил
не-ASCII байты раньше смены страницы. Починено в `build.bat` 2026-09-22: контрольная
сборка из чистого bash (`cmd //c ".\build.bat"`) даёт exit 0 и ZIP — обёртка больше не нужна.

**Правило.** Для релиза RELEASE_PROCESS рекомендует прямые команды вместо `build.bat`
(он в конце делает `start ""` приложения). Шапка `chcp 65001` в `build.bat` обязательна
и не удаляется: без неё bat снова молча умирает из bash/CI на копировании штампа.

---

### 43. Тест с местным временем автора роняет CI в другом поясе (ВЫСОКИЙ)

**Симптом.** Локально 2402/2402 зелёные, а CI-релиз падает на шаге Test:
`DeviceLastSeenHintTests.Yesterday_ShowsLocalTime` ожидал «10:10» — литерал,
верный только в поясе автора (+03). Раннер GitHub — UTC → рендер «07:10» → красный.

**Причина.** `DeviceLastSeenHint.Text` рендерит локальное время машины
(`ToLocalTime()`) — по дизайну (админ смотрит чип «последняя связь» в СВОЁМ
времени). Тест же строил ожидание от фиксированного `+03:00` в строке даты:
на машине в другом поясе ожидание и рендер расходятся. Локальный полный прогон
такой тест не ловит никогда — прогон идёт в том же поясе, что и написание.

**Решение.** Оба `DateTimeOffset` строить от `TimeZoneInfo.Local` (инстанс —
его собственное смещение, ожидание «вчера 23:59» стабильно в любом фиксированном
поясе). Общее правило: в тестах времени литерал смещения не должен кодировать
пояс автора; ожидание выводится по построению, а не хардкодится. Поймано
релизным CI-прогоном v3.53.0 (единственный красный из 2402).

**Правило.** CI-гейт релиза обязан быть зелёным ДО пуша тега, когда это возможно;
зонно-чувствительные тесты проверять глазами на CI-поведение (UTC), а не только
локально. Рантайм-код «локальное время машины» — легитимен, ломаются только тесты.

### 44. «Оптимизация под отключённые анимации» молча отключает применение темы (ВЫСОКИЙ)

**Симптом.** После обновления до 3.53.0 на «некоторых устройствах» приложение
осталось в дефолтной палитре `Brushes.xaml`: у тёмных пользователей — светлый
контент с тёмным сайдбаром и панелью «ИТОГО» (светлые дефолты неотличимы от
светлой темы, виден только сайдбар). На машинах разработки не воспроизводится.

**Причина.** `ThemeService.ApplyTheme` создаёт кисть slow-path **в старом
цвете** — «чтобы анимация интерполировала от старого к новому» — а цвет двигает
только `BeginAnimation`. Коммит a11y (ad39a0d) добавил gate
`animate = … && !Motion.ReducedMotion` (системная настройка «отключить
анимации», `ClientAreaAnimation=false`): на таких машинах ветка анимации
отключилась, и кисть навсегда осталась в старом цвете. Взаимодействие двух
правильных по отдельности правок — гейт и «стартовый цвет для интерполяции».

**Решение.** Без анимации кисть создаётся сразу в целевом цвете:
`(animate && oldColor.HasValue) ? old : new`. Страж —
`ThemeApplyTests.ApplyTheme_SetsBrushesToTargetColors_WithoutAnimation`:
прогоняет `ApplyTheme(TimeSpan.Zero)` в обе стороны и проверяет `Surface`/`SidebarBg`
в целевых цветах; проверен на откате фикса — ловит баг.

**Правило.** Любая ветка «если анимация отключена — делаем без анимации»
обязана гарантир��вать конечное состояние тем же способом, что и анимированная
(у анимации есть `To=`; у не-анимации ничего нет). Аудит: искать пары
`BeginAnimation` + условный гейт вокруг создания начального значения.
...
| Обновления | Технический разбор из CHANGELOG уезжает в «Что нового»: генератор берёт title/changes из CHANGELOG вместо курированной записи json; битая кодировка заголовка (двойной mojibake) остаётся незамеченной без прогона (§39) | ВЫСОКИЙ |
...
2026-09-23 — §18 скорректирован: текст «На завод» и его тесты удалены вместе с кнопкой (v3.54); заводские размеры (−20 мм) остаются в модели Anwis и объяснении режимов — при работе в «заводской» зоне не путай удалённый генератор текста с живой размерной моделью.

2026-09-22 (v3.53.0) — обновлено содержимое (sync-last-verified.ps1, CONTROL#13).

2026-09-21 (v3.53.0) — хвост 3.40.0–3.43.2 переписан пользовательским языком, заголовок 3.41.0 восстановлен из двойного mojibake, порог стража опущен до 3.40.0 (страж покрывает журнал целиком), вычищен и releases.json 3.40.0–3.43.0; §39.
