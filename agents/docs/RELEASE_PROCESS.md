...
В индекс при этом уходит ТОЛЬКО строка записи (не файл целиком): иначе в коммит
попадали бы незастейдженные правки чужой задачи — сценарий и замер в GOTCHAS §41.
...
`update-log.json` читает пользователь (окно «Что нового» и вкладка «Обновления»),
поэтому в пунктах НЕ бывает: путей и имён файлов, токенов и идентификаторов
(`Radius.Card`, `Motion.Run`), замеров в пикселях, счётчиков прогонов и «pass»,
hex-цветов, ссылок на GOTCHAS, названий классов платформы (WPF/XAML/DataGrid…)
и слов «тест», «сборка», «оверлей», «токен», «бюджет», «ратчет», «харнесс».
Технический разбор пишется в `CHANGELOG.md` — он для разработчика.

Проверка автоматическая: `MosquitoNetCalculator.Tests/Services/UpdateLogVoiceTests.cs`
роняет сборку на техжаргоне в записях начиная с версии 3.50.0 и держит бюджет
карточки (≤ 14 пунктов, ≤ 480 знаков на пункт). Разбор — GOTCHAS §39.

`tools/release/generate-update-log.ps1` берёт версию и дату из CHANGELOG,
а title/changes — из уже курированной записи json (человеческий текст НЕ затирается
техническим). Если записи версии в json ещё нет, скрипт создаст её из CHANGELOG как
ЧЕРНОВИК и напечатает warning: вычитай текст перед релизом, иначе страж упадёт.
