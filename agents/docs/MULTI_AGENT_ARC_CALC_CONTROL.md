...
Даты «Last verified» вручную НЕ проставляются (GOTCHAS §40): их ставит
`agents/scripts/sync-last-verified.ps1` — на коммите через локальный `pre-commit`
(`agents/scripts/install-git-hooks.ps1`) или вручную перед финальным отчётом.
...
6. **Проверка после правок docs:** `what-to-update.ps1` → обновить перечисленные →
   `validate-docs.ps1` (ритуал §9). Дату «Last verified» не пиши руками — её ставит
   `sync-last-verified.ps1` (на коммите через `pre-commit`; §40).
