...
- В гриде заказов рядом с названием показывается бейдж **«АК»** (`IsAnticat`); клик открывает тёмный попап с `AnticatToolTip` (ставка, площадь, сумма надбавки, «учтена в сумме»). Свойства: `AnticatSurchargeTotal`, `AnticatToolTip`.

---

## Импост (авто-надбавка +200 ₽/м.п. по ширине)

Для оконных москитных сеток импост ставится **автоматически**, когда полотно
достаточно большое. Убрать его нельзя — флаг чисто производный.

### Критерии (по РАСЧЁТНЫМ размерам)

| Условие | Импост |
|---------|--------|
| Ширина ≥ 500 мм **ИЛИ** высота ≥ 1500 мм | ✅ ставится |
| Ширина < 500 мм **И** высота < 1500 мм | ❌ нет |

- Оцениваются **расчётные** размеры (`OrderItem.Width`/`Height` — те же, что
  идут в площадь, цену и КП; для Anwis — после коррекции режима: ББ60 W+2/H−30,
  ББ70 W−2/H−30, Проём W+20/H+20). Для не-Anwis товаров расчётные = введённым.
  Пример (владелец, 2026-08-24): Anwis ББ60 при вводе 500×1500 хранит 502×1470 →
  критерий и сумма от расчётной ширины **502**: 502 ≥ 500 → импост 200 × 0,502 =
  **100,40 ₽** (не 100 от введённых 500).
- **Смена режима сетки пересчитывает импост**: расчётная ширина зависит от
  режима (ввод 600: ББ60 → 602, ББ70 → 598), поэтому при смене режима в уже
  добавленной строке сумма импоста обновляется автоматически через
  сеттер `AnwisSizeMode` → `Recalculate()`. Критерии считаются от тех же
  расчётных размеров.
- **Дверная сетка исключена полностью** — импост не участвует в её расчёте
  (ни авто-установка, ни сумма).

### Сумма (всегда от ШИРИНЫ)

```csharp
// Константа ставки
OrderItem.ImpostRatePerLinearMeter = 200;   // ₽ за 1 м.п. по ширине

// Погонные метры импоста — ВСЕГДА от расчётной ширины, независимо от того,
// какой критерий (ширина или высота) сработал:
ImpostLinearMeters = Math.Round(Width / 1000.0, 3);

// Единый хелпер суммы — его используют И строка, И превью быстрого добавления:
static double ImpostSurchargeFor(double calcWidthMm, double quantity) =>
    Math.Round(ImpostRatePerLinearMeter * Math.Round(calcWidthMm / 1000.0, 3) * quantity, 2);

// В Recalculate() надбавка добавляется к сумме строки (как антикошка):
if (HasImpost)
...
| Сетка (расчётные размеры) | Критерий | Импост |
|---------------------------|----------|--------|
| 600×800 | ширина | 200 × 0.60 × 1 = **120 ₽** |
| 600×1600 | оба | всё равно 200 × 0.60 × 1 = **120 ₽** |
| 600×800 × 3 шт | ширина | 200 × 0.60 × 3 = **360 ₽** |
| Anwis ББ60: ввод 500×1500 → расчёт 502×1470 | ширина | 200 × 0.502 × 1 = **100,40 ₽** (не 100 от ввода) |
| Anwis ББ60: ввод 430×1510 → расчёт 432×1480 | ни один | **0 ₽** (высота расчёта 1480 < 1500) |
| Anwis ПП: ввод 400×1510 → расчёт 400×1510 | высота | 200 × 0.40 × 1 = **80 ₽** |

- `HasImpost` — производное свойство (`ProductCatalog.IsImpostApplicable(Name)
  && (Width >= 500 || Height >= 1500)`), **не сериализуется** → убрать
  импост физически невозможно (см. GOTCHAS#18); при загрузке заказа
  пересчитывается из хранимых (расчётных) размеров.
- Критерии и сумма вынесены в статические хелперы `OrderItem.ImpostApplies` /
  `OrderItem.ImpostSurchargeFor` — **единый источник** для строки (`Recalculate`)
  и превью быстрого добавления: превью считает расчётные размеры через
  `AnwisSize.ОтВвода(...)` с выбранным режимом и показывает «… + Импост N ₽ = сумма»,
  равную сумме добавленной строки (площадь там тоже округляется до 3 знаков).
- Применяется к товарам `ImpostApplicableProducts`: **Anwis, На навесах,
  Оконная на метал. крепл.**
- Сумма импоста входит в `Total`, а значит в `TotalWithDeduction`, КП, итоги
  и текст «На завод».
- В `DisplayName` добавляется суффикс `(Импост)` (после `(Антикошка)`), поэтому
  в DataGrid, КП и секциях «На завод» квалифицирующиеся сетки помечаются
  отдельно.
- В гриде заказов рядом с названием показывается бейдж **«ИМ»** (`HasImpost`);
  клик по нему открывает тёмный попап с `ImpostToolTip`: **расчётная** ширина в мм,
  м.п., сумма надбавки и пометка «учтён в сумме строки» (клик вне/повторный клик
  закрывает). Свойства: `ImpostSurchargeTotal`, `ImpostToolTip`.
