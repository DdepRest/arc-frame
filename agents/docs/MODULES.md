...
| `PageSelection.cs` | Единая логика выбора страниц КП (All/Single/Range) для печати и PDF-экспорта: клэмпинг границ к фактическому числу страниц, разворот диапазона при from>to. Используется `FixedDocumentBuilder` и `PdfExportService` — оба канала никогда не расходятся в том, какие страницы попадают в вывод. |
| `ProductionStampImage.cs` | Единая точка доступа к изображению печати «В ПРОИЗВОДСТВО» (физическая печать `FixedDocumentBuilder` + PDF `PdfExportService`): поиск файла рядом с exe, размеры (пропорции 2:1, ~70×35 мм), левое смещение `LeftOffsetDip`/`LeftOffsetMm` = 16 мм − внутренний прозрачный отступ картинки (≈1.3 мм) → ≈14.7 мм, чтобы видимая рамка оттиска начиналась ровно на левой линии контента КП, верхний отступ `TopOffsetDip`/`TopOffsetMm` (~1.6 мм) — единый для печати/предпросмотра/PDF, политика «нет файла → пропустить печать». При физической печати позиция задаётся `RenderTransform` (TranslateTransform), а НЕ Canvas.Left/Top: отсоединённый FixedPage не размещается, и XPS-сериализация (путь печати) теряет Canvas-смещения — элемент уезжал в (0,0) на бумаге. | Позиция/размер/смещение печати и политика при отсутствии файла — одинаковы для печати, PDF и предпросмотра. |
| `OrderTemplateService.cs` | Система шаблонов заказа (v3.54, замена «На завод»): каталог шаблонов («Окно» доступен, «Балконный блок»/«Рама»/«Француз» — заглушки), состояние чек-листа, матрица валидации, резолв цен и сборка параметров позиций. Чистая логика без WPF. | Добавление позиций выполняет UI атомарно (один PushUndo); заводы формулы не затрагиваются — только параметры для `CalculationViewModel.AddItem`. |
| `UpdateService.cs` | Проверка обновлений, скачивание ZIP (основной канал с фолбэком на запасной `mirrorUrl` из манифеста: сетевой сбой после ретраев или несовпадение SHA-256 → повтор через зеркало, та же проверка SHA-256; оба канала недоступны → понятное сообщение без технических деталей — `BuildDownloadFailureReason`), верификация SHA-256, запуск watchdog. Оркестрация через `RunUpdateFlowAsync`: тост ручной проверки содержит ТОЧНУЮ причину сбоя манифеста (таймаут/HTTP-код/сеть) вместо обезличенного «Не удалось получить список обновлений». | ManifestUrl, логика сравнения версий, имена файлов — влияет на автообновление. |
...
| `UpdateDownloader.cs` | Скачивание ZIP с прогрессом и ретраями (3 транзиентных повтора с backoff), валидация URL до запроса (`IsSupportedUrl` — только абсолютный HTTP(S), иначе понятная ошибка и HTTP-вызова нет). | Число ретраев/задержка — влияет на скорость фолбэка на зеркало. |
...
| `DeviceLastSeenHint.cs` | Чистая функция «время последнего отчёта устройства» → текст подсказки чипа («· вчера 09:10», «· 3 дн. назад») для отчётов старше 12 ч; свежим устройствам подсказка не нужна. | Порог 12 ч и формат даты — вид чипов устройств админки. |
| `AdminPanelLogic.cs` | ЧИСТАЯ политика панели «Обновления» (без WPF и IO, единственный владелец фраз и правил «что показывать»): пустые состояния (`IsNoData/IsFilterNoMatch/EmptyTitle/EmptyHint`), подсказка сводной карточки (`SummaryHint`), маркер «недавно отвязаны» (`JustUnboundText/JustUnboundReturned`), тексты напоминаний (`SingleReminderText/BulkReminderText/VersionLine/FallbackDownloadUrl`), шапка «обновлено … · авто …» (`RefreshStatusText`). | Тексты/пороги — поведение панели; контрол только применяет результаты. Владельцы состояния зафиксированы в шапке модуля (gist — OfficeReportService, строки — OfficeStatusCalculator, vis — контрол). |
...
| `ActionBarControl` | Нижняя панель: итоги, кнопки Печать, Шаблоны, Сохранить, Обновления. |
...
| `AdminPanelControl` | Админ-панель — контейнер секций (вкладки): «Обновления» (статусы версий) и «Статистика» (кол-во заказов). Новые секции = новый TabItem + свой список строк. Список офисов сгруппирован по статусу, есть поиск и чипы-фильтры; быстрые действия: «Отправить напоминание» (одно сообщение по всем устаревшим) и «Отвязать выбранные» (чекбоксы на устройствах → удаление файлов из gist одним PATCH). Авто-рефреш каждые 15 мин, пока панель открыта; каждый рефреш шлёт свой отчёт и тихо чистит устаревшие (>24 ч) дубли/чужие офисы gist. Разбит на partial: `.xaml.cs` (рефреш), `.Filter.cs` (поиск/фильтр/группы), `.Actions.cs` (быстрые действия); фразы и правила «что показывать» — чистый `AdminPanelLogic` (см. Services). Селекция чекбоксов отвязки живёт на модели (`OfficeDeviceRow.IsChecked`, TwoWay) — видимое всегда совпадает с тем, что будет отвязано. | Читает gist — без сети показывает "нет связи"; формат отчёта расширяется обратносовместимо. |
| `AdminPasswordWindow` | Окно ввода пароля админ-панели (вшитый пароль, единый для всех офисов). | — |
| `TemplatesWindow` | Модальное окно «Шаблоны» (v3.54): витрина карточек-шаблонов → чек-лист позиций с полями → атомарное добавление в заказ (один Undo-шаг). Строки чек-листа строит code-behind. |
...
| `ai-system-prompt.md` | Системный промпт AI-ассистента. |
| `tessdata/` | Данные для нативного OCR (Tesseract, rus/eng). |
...
| `OrderTemplateServiceTests.cs` | Каталог шаблонов, матрица валидации, резолв цен, сборка позиций. |
| `TemplatesWindowTests.cs` | STA-страж: окно «Шаблоны» конструируется с реальными темами (ловит отсутствующие ресурсы разметки — в рантайме ошибку конструктора окна глотает DispatcherUnhandledException, и кнопка выглядит «мёртвой»). |
...
2026-09-21 (v3.53.0) — в таблицу тестов добавлен `Services/UpdateLogVoiceTests.cs` (страж голоса пользовательской записи об обновлении).
