...
## Unreleased — Installation amount sign input

- Исправлен popup монтажа: при сумме `0` выбор «−» теперь сохраняется до ввода числа, а явно введённое отрицательное значение (`-500`) корректно преобразуется в signed-корректировку. Некорректные, бесконечные и переполняющие значения не записываются. Формулы расчёта и `TotalWithDeduction` не изменялись; добавлены регрессионные тесты преобразования знака.

## Unreleased — Docs maintenance

- Синхронизированы даты «Last verified» в `docs/arc/*.md` с git-историей (CONTROL#13): подняты даты в `CALCULATION_LOGIC.md`, `GOTCHAS.md`, `PROJECT_OVERVIEW.md`, `REFACTORING_PLAN.md`, `ux-refactor-plan.md`, `CALCULATION_TEST_CASES.md`, `update-notification-rework-spec.md` после сверки содержимого с текущим состоянием (v3.47.3, 1511/1511 tests pass). Код не менялся.

## Unreleased — Stable progress tests

- Исправлена нестабильность тестов загрузчика обновлений: проверки прогресса используют синхронный `IProgress<int>` вместо асинхронного `Progress<int>`, поэтому `100%` проверяется после фактического завершения callback и не зависит от гонки dispatcher-контекста. Production-код загрузки не изменён.

## Unreleased — Network printer reliability

- **Исправлена отправка КП на выбранный сетевой принтер:** список и печать теперь используют один и тот же экземпляр `PrintQueue`, полученный из `Local + Connections`; выбранная очередь больше не пересоздаётся по строковому имени перед отправкой. Это сохраняет UNC/серверный контекст подключённого принтера и исключает подмену на физический USB-принтер. Для явно выбранной очереди fallback на системный принтер запрещён; default используется только при отсутствии выбора. Если сохранённая очередь исчезла, приложение не выбирает первый принтер автоматически — печать разблокируется только после явного выбора пользователем.

## Unreleased — AI Agent Mode

- **Скрыты технические сведения AI:** в открывающихся AI-панелях больше не показываются выбранная модель, провайдер и телеметрия запроса; внутренний выбор моделей и метрики продолжают работать без отображения в пользовательском интерфейсе.

- **AI-раздел временно отключён:** пункт «AI Ассистент» открывает интерфейс только для просмотра с блокирующей плашкой «В РАЗРАБОТКЕ»; ввод, отправка, очистка чата и действия плана недоступны. Пункт меню «AI Ассистент — API ключ…» оставлен видимым, но отключён и диалог ключей не открывает. Расчётная логика и AI-сервисы не изменялись.

- **AI Agent Mode:** AI больше не меняет расчёт напрямую — каждая изменяющая команда сначала превращается в план действий (`AiActionPlan`), проходит локальную проверку (`AiPlanValidator`), показывается пользователю карточкой-предпросмотром и выполняется только после нажатия «Выполнить» (или отменяется). В пакете может быть несколько шагов (`mode/steps` из LLM или несколько локальных команд).
- **Атомарное выполнение и отмена:** весь пакет плана применяется одним снимком Undo — кнопка «Отменить действие» в чате возвращает заказ ровно в состояние до выполнения плана; частичный сбой шага откатывает весь пакет (`AiPlanExecutor`, guarded undo в `MainWindow.AI.cs`).
- **Slash-команды без интернета** (`AiLocalCommandRouter`): `/товары`, `/цены`, `/итоги`, `/статус`, `/последняя`, `/отменить`, `/повторить`, `/очистить`, `/объясни` — работают мгновенно и не тратят токены; `/очистить` при пустом расчёте отвечает без лишнего подтверждения.
- **Богатый контекст заказа** для LLM (`AiOrderContextBuilder`): AI получает актуальные позиции, цвета, размеры, цены и итоги заказа — предугадывает типовые запросы («сделай сетку» → сетка с дефолтными параметрами) и задаёт только необходимые уточнения.
- **Объяснение расчёта** (`/объясни`, `AiExplanationContextBuilder`): AI объясняет фактический расчёт по реальным итогам заказа (материалы, монтаж, откосы), а не придумывает формулу.
- **Защита от двойного выполнения:** каждый план и сообщение получают `PlanId`/`MessageId`/`RequestId`; повторный клик, regenerate или дублирующий запрос не может выполнить уже выполненный/отменённый план.
- **Телеметрия запросов** (`AiTelemetryService`): в памяти накапливаются метрики — запросы, попытки, fallback-переключения провайдера.
- **Golden-набор AI-тестов:** `AI/golden-cases.json` + `AiGoldenCaseTests` покрывают реальные фразы менеджеров («Сделай сетку», «Добавь 2 окна…», «Удали последнюю» и т.д.) — одиночные и пакетные сценарии.
- **Исправлен краш при отрисовке карточки плана** (`XamlParseException: StaticResourceHolder`): кнопки «Отмена» и «↩ Отменить действие» в `AiAssistantControl.xaml` ссылались на `{StaticResource DialogOutlineButton}`, который был определён только в локальных ресурсах окна `AiApiKeyDialog` — при появлении плана (включая восстановленный из истории чата) WPF падал. Стили `DialogOutlineButton`/`DialogPrimaryButton` перенесены в общий `Themes/ButtonStyles.xaml`; добавлен регрессионный тест, проверяющий разрешимость всех StaticResource-ключей контрола.
- **Исправлен баг gensymbols.ps1:** символ `g` в строке формата `(gensymbols.ps1)` интерпретировался .NET как спецификатор эры («наша эра» + мусор в `Last generated`); литерал вынесен из формата.

## Unreleased — AI catalog and navigation UX

- Надёжность AI-запросов: при сбое провайдера каждая модель теперь ретраится до 3 раз с экспоненциальной паузой (429/5xx/сетевые ошибки/пустой поток), а в цепочку фолбэка всегда добавляется минимум одна бесплатная модель NVIDIA — даже если пользователь вручную выбрал только OpenRouter-модели. Ошибка «Все доступные модели недоступны» больше не хардкодит имя провайдера (`AiAssistantService`: `MaxAttemptsPerModel`, `RetryDelayMs`, `EnsureNvidiaFallback`).
- Интерактивная карточка уточнения показывает только товары, релевантные запросу: «Сделай сетку» → в списке «Тип» только сеточные изделия (Anwis, На навесах, Оконная на метал. крепл., Дверная сетка); «отлив»/«козырёк»/«короб» и т.д. — соответствующий товар, а не весь каталог. Без ключевых слов — полный каталог (`AiClarificationForm.FilterProductsForRequest`).
- Когда AI отвечает уточнением вместо добавления товара («Сделай сетку» → «Уточните: тип, размеры, цвет…»), в пузыре ответа появляется интерактивная карточка «Заполните параметры»: выпадающие списки типа/цвета/режима Anwis/монтажа + поля размеров и количества. Кнопка «Добавить в расчёт» создаёт команду AddItem напрямую, без повторного запроса к LLM. Детект уточнения по маркерам («Уточните», «Укажите», «Выберите», «⚠ режим/размер/глубина»); форма учитывает каталог товаров и цветов (`AiClarificationForm`, `Models/AiClarificationForm.cs`).
- Автовыбор AI теперь действительно ставит модель, выбранную по типу задачи, первой; отмеченные вручную модели используются как резерв, а ручной режим сохраняет заданный порядок.
- Исправлено обрезание кнопок «Отмена» и «Сохранить» в настройках AI: для нижней панели зарезервировано отдельное место внутри фиксированного окна.
- Во время ответа AI поверх области чата постоянно виден анимированный статус «Думает…»/«Печатает…», который скрывается после завершения запроса. Индикатор привязан напрямую к `IsBusy`, а пустой пузырь до первого токена показывает собственный индикатор «Думает…» (связка `IsThinking`) — статус не зависит от таймингов code-behind.
- Футер настроек AI («Отмена»/«Сохранить») закреплён отдельной строкой у нижней границы окна, а вся форма помещена в прокручиваемую зону — кнопки физически не могут быть обрезаны при любом масштабе DPI.
...
- Проверки: `dotnet build MosquitoNetCalculator.sln -c Debug` и `dotnet test MosquitoNetCalculator.sln -c Debug` — успешно, **1498 тестов** (+70 новых для AI Agent Mode + регрессионный тест StaticResource).
