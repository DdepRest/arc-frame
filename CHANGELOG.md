...
## Unreleased — Сумма монтажа в таблице заказа и на печати КП (v3.48.3)

- **Сумма монтажа видна у каждой строки:** в колонке «Монтаж» теперь не только глиф режима, но и деньги всей строки: ставка × м.п. длины × Кол-во. В гриде сумма идёт мелким текстом под кнопкой-глифом («+750» / «−500»); в печати КП и PDF-экспорте — в той же ячейке рядом с глифом («✓ +750», «В −500»). Ячейка «—» и нулевые ставки выглядят как раньше. Новые свойства `InstallationRowTotal` / `InstallationAmountDisplay` / `KpInstallationCellText` (`OrderItem.Installation.cs`) — только отображение, расчётные формулы не менялись.
- **Ширины колонок КП перебалансированы** без новых колонок: «Монтаж» 0.065→0.095; компенсация: «Наименование» −0.01 (переносится по дизайну), «Площ./Дл.» −0.01, «Сумма» −0.01; **«Цвет» не тронут** — гард-тесты требуют рендер «Коричневый» без шринка. Сумма долей осталась 1.0. PDF-колонки: Цвет 1.5→1.2, Монтаж 1.5→1.9.
- **Тесты:** +3 в `OrderItemTests` (кейс владельца 350×1000 → «+750»; пусто для «—»/нулевых ставок; −1000 при «Без монтажа» ×2); `Check7_Print_Renders_V_Konstr_Glyph_For_Mode2` обновлён под новый формат печати («В −500»). Полный прогон **2044/2044 pass**, сборка 0 warnings.
