...
- **Тесты:** +5 `AiAssistantViewModelTests` (кап 100/под капом, пакетная подгрузка, сброс при ClearChat) + 2 регрессии (`UxStateRegressionTests.Themes_HaveNoLiveDropShadow_InTriggersOrTemplates` — запрет эффектов в Themes; `AiAssistantControlTests` — запрет per-template анимации + связка капа) + рендер-регрессия `UpdateCardStripeTests`. Итого **2280/2280 pass**, сборка 0/0, UI smoke dark+light зелёные.
