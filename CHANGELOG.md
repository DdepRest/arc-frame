...
- **Полоса типа версии повторяет скругление карточки** (владелец: «дизайн скруглённый, а полоска квадратная»): на карточке «Истории обновлений» полоса была отдельным прямоугольником `Width=4` и закрашивала скруглённый угол. Теперь это левая граница внутреннего `Border` (`BorderThickness="4,0,0,0"`, `CornerRadius="11,0,0,11"`) — рамка Border обводит скруглённый контур, поэтому полоса обходит угол по дуге карточки. Проверено пиксельным тестом `UpdateCardStripeTests` (рендер реального `UpdatesTabControl`): в углах карточки пикселей полосы ноль, на кромке — есть (на старом markup тест падает: 8 пикселей в углу).
...
- **Тесты:** +5 `AiAssistantViewModelTests` (кап 100/под капом, пакетная подгрузка, сброс при ClearChat) + 2 регрессии (`UxStateRegressionTests.Themes_HaveNoLiveDropShadow_InTriggersOrTemplates` — запрет эффектов в Themes; `AiAssistantControlTests` — запрет per-template анимации + связка капа) + рендер-регрессия `UpdateCardStripeTests` + 4 регрессии `TestAppThemesTests` (лечение «отравленной» статики WPF, повторы чтения словаря, текст ошибки с путём и причиной). Итого **2284/2284 pass**, сборка 0/0, UI smoke dark+light зелёные.
