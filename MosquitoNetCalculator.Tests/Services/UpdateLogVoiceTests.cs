...
namespace MosquitoNetCalculator.Tests.Services
{
    ...
    /// на 3548 знаков и 4 пункта на 942 знака; 3.48.3, 3.48.4 и 3.48.6 уже были
    /// пользовательскими — их не трогали.
    /// </para>
    ///
    /// <para>
    /// Затем — ряд 3.44.0–3.47.4 (11 записей): два рекордсмена — 3.44.0 (66 пунктов,
    /// 8987 знаков) и 3.44.2 (69 пунктов, 13 162 знака) дублировали друг друга и
    /// вели диалог о тестах и колонках печати; 3.45.0 был рефакторинг-журналом с
    /// именем файла в заголовке. Теперь 3.44.0 — 13 пунктов на 1916 знаков, 3.44.2 —
    /// 7 на 1191, 3.45.0 — 3 на 373; 3.44.1 и мелкие 3.46/3.47 переведены в тот же
    /// голос.
    /// </para>
    ///
    /// <para>
    /// Наконец — хвост 3.40.0–3.43.2 (11 записей): 3.41.0 держал 130 пунктов на
    /// 36 265 знаков, а его заголовок был перечитан в чужой кодировке — «РЈР»СѓС‡С€РµРЅРёСЏ»
    /// вместо «Улучшения»; 3.43.0/3.43.2 — по 32/33 пункта с именами тестов
    /// («AddNewUpdate_ZeroIsLatestFrame_neverObserved») и счётчиками (709/711,
    /// 789/789); в 3.42.1 — пути вида %AppData% и .bat-скрипты. 3.40.0–3.40.4 были
    /// почти чистыми — убраны «opt-in» и потерянные «до/после». Порог опущен до
    /// 3.40.0 — самой старой записи журнала: теперь страж видит ВСЁ, что лежит в
    /// update-log.json, и новое попадание техжаргона в старую запись поймает.
    /// </para>
    ///
    /// <para>
    ...
    {
        /// <summary>
        /// С какой версии действует правило — самая старая запись журнала
        /// (3.40.0): хвост 3.40.x–3.43.x переписан на язык результата, и страж
        /// теперь покрывает update-log.json ЦЕЛИКОМ. Дальше опускать некуда;
        /// для будущих записей порог не трогаем.
        /// </summary>
        private static readonly Version Cutoff = new(3, 40, 0);
