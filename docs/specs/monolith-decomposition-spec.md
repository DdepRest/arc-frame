...
Проект A.R.C. Frame (WPF .NET 8, MosquitoNetCalculator) вырос до 60+ классов, но ряд ключевых файлов остался монолитным — в них смешано 5–10 ответственностей. Это замедляет навигацию AI-агентов и человека, повышает риск конфликтов при параллельной работе и затрудняет документирование в `agents/docs/`.
...
Разбить монолиты на части, сохранив **100% поведение** (публичные API не меняются), адаптировать тесты под новую структуру, обновить `agents/docs/*` так, чтобы каждый файл имел однозначное описание в MODULES.md/INTENTS.md/SYMBOL_INDEX.md.
...
#### 4B. Новый документ: `agents/docs/REFACTORING_GUIDE.md`
