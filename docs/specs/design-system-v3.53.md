...
4. **Системная настройка «отключить анимации» уважается:** `Motion.Run(storyboard)`
   проверяет `SystemParameters.ClientAreaAnimation` и при выключенных анимациях
   не запускает Storyboard. Состояние при этом применяется — мгновенно.
