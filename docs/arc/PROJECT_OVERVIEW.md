...
| Архитектура | win-x64 |
| Единственная внешняя зависимость | Microsoft.Web.WebView2 (для предпросмотра печати) |
...
- **Коммерческое предложение** — HTML-шаблон с SVG-чертежами, печать через WebView2.
