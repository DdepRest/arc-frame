...
| **Рекомендация** | 1. Запустить `build.bat` → получить ZIP. 2. Запустить `tools/release/_get_hash.ps1` или `Get-FileHash -Algorithm SHA256`. 3. Записать реальный `size` (в байтах) и `sha256` (lowercase hex) в `releases.json`. 4. Закоммитить. |
...
| **Файл** | `agents/docs/CURRENT_STATE.md`, раздел "Source files" |
